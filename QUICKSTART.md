...
```bash
webmuxd new --id work --port 7900 --dsf 2 --quality 95
```

**`--dsf` 不是"越大越清晰"的旋钮,它只用来匹配观看端的 dpr。**
普通屏(dpr=1)上填 2,实测锐度反而**低 18%**,还多花 2.6 倍带宽 ——
原生尺寸渲染时的抗锯齿是针对那个像素网格优化过的,放大再缩回来等于把它
重新平均了一遍。
