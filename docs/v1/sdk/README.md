...
`sess.active` **就是"浏览器现在放在前台的那一页"**,是观测出来的
([api/tabs.md §5](../api/tabs.md))。所以它和人眼看到的永远是同一页 ——
页面自己开了个前台 tab,它跟着换。

> 0.18.0 之前这儿写的是"不是观测出来的,是 sessiond 记的,唯一会漂的是人按
> `Ctrl+Tab`,下次就对齐回来"。**那两句都是错的** —— 漏掉了最常见的那种
> (页面 `target=_blank`),而且不会自愈:在下一次 `activate` 之前,
> 人一直看着一页、命令一直打在另一页上。
