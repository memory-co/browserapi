...
> **正在换一套编号。** `01`–`13` 是按落地顺序攒出来的,读起来像流水账 ——
> 一个新来的人得读完七篇才知道这东西整体长什么样。新的一套按**字母**编,
> 按"要理解这个系统,该先知道什么"排。**第一批只有 a**,其余仍在 `01`–`13`,
> 迁一篇改一篇,不一次性搬。
...
| 文件 | 内容 |
| --- | --- |
| [a-frame-vs-tmux-ttyd.md](a-frame-vs-tmux-ttyd.md) | **整体定位** —— tmux 那一半照搬、ttyd 那一半搬不动(终端的画面就是它的语义,浏览器的不是);三条 WebSocket 各走什么协议、为什么不能合成一条;光标同步当例子:终端白给的东西浏览器要自己造 |
