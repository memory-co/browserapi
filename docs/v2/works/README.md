...
| **[c](c-pixels.md)** | 像素从哪来 | 第一条来源(CDP 截屏)的结构上限 → 为什么引入第二条 → **两者产出的本来就是同一种东西**,于是「像素从哪来」是一个干净的接缝。§13 是第三条(rrweb):**它连像素都不传**,由观看端重排 DOM —— 接缝之上仍不用改,但坐标不再天然对齐。附:xpra / Xvfb / Xorg 各是什么、要探什么 |
...
- ❌ **不做 H.264 / VP8 / WebRTC。** 不是做不到 —— xpra 那边编码器是现成的。
  ...
  不急的原因是还没量到它值多少([c §14](c-pixels.md#14-还没定的))。
...
(唯一的例外是 [c §13](c-pixels.md#13-第三条来源rrweb--它不传像素) 那条 rrweb 来源,它写明了是设计。)
...
| **第三条来源** | `/channel/rrweb` **只有设计,没有实现** —— 是这套文档里唯一一处不描述现状的内容,四项待办列在 [c §13.6](c-pixels.md#136-现状) |
