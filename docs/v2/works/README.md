...
| 文件 | 内容 |
| --- | --- |
| [e-client.md](e-client.md) | **观看端的客户端** —— 这个位置在 ttyd 侧由 xterm.js 承担,这里**没有现成实现**;它做的不是渲染而是协议;分三层,**只有协议层无法由外部实现**;§6 是**通道模型**:每条通道对应一个上游系统(`/cdp` ↔ Chromium,`/xpra` ↔ xpra),声明自己能提供什么,由固定的优先级表仲裁 —— **帧优先取 xpra、光标优先取 CDP**;可并列加第三条,但**输入永远只走 `/cdp`** |
