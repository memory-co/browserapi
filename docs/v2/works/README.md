...
| **[c](c-pixels.md)** | 像素从哪来 | 第一条来源(CDP 截屏)的结构上限 → 为什么引入第二条 → **两者产出的本来就是同一种东西**,于是「像素从哪来」是一个干净的接缝。§13 拿 **rrweb 与 Playwright trace** 验了这条判据:**两个都不合格,而且方式不同**。附:xpra / Xvfb / Xorg 各是什么、要探什么 |
