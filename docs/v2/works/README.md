...
| **[l](l-extension.md)** | sidecar 变成 Chrome 扩展? | **实测四条全通**:无头和 `--kiosk` 都装得上 MV3 扩展、`chrome.tabs.onActivated` 收得到 CDP 发的切换、`onCreated` 一条就带 `active`(普通左键 `true`、**Ctrl+左键 `false` 且不发 activated**)、rrweb 在**隔离世界**里录得动而**页面主世界完全干净**。但 `chrome.windows.onFocusChanged` **一次都没触发** —— 它管窗口不管 tab,选错了。连 `open-shim` 都换得掉,而且**更好**:`chrome.tabs.move` 把 popup 搬回主窗口,**一个字都不注进页面**、不需要白名单(今天那个白名单里 `attributionsrc` 实测**照样开出真窗口**)、`noopener` 的 `null` 自动保住。真正换不掉的只有 `remote`(装不了扩展)—— **问题不是可不可行,是要不要多一条腿** |
