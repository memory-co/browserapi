...
| 文件 | 内容 |
| --- | --- |
| [f-tabs.md](f-tabs.md) | **tab:外挂的 bar,和真的那个是同一份数据** —— 它不是副本,是同一张 target 表的另一种呈现,因此**没有可漂移的东西**;xpra 那条上可以直接用眼睛核对(去掉 `--kiosk` 就能看见真的 bar);`active` 从一本账变成当前事实 |
...
| [f-tabs.md](f-tabs.md) | tab 外挂模式一字不改;`active` 从两份真相合成一份 |
