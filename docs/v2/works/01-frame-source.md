...
**一句话**:画面不再来自镜像里的 VNC,而是 **webmuxd 自己产**、用 `Input.*`
自己收输入。VNC 这条路整条砍掉,不留开关。

> **这一篇讲的是第一条实现:CDP `Page.startScreencast`。**
> 0.7.0 起默认换成了 xpra([11](11-xpra.md) · [12](12-xpra-client.md)),
> screencast 成了明确的退路。**但本篇的论证一条都不用改** ——
> 它论的是"画面该不该归我们",而不是"该用哪个编码器";
> 换到 xpra 恰恰是这个判断成立之后才可能发生的事(§5 末尾)。
...
> **待补(仍然欠着)**:这条目前是主观流畅度对比,还缺**和 VNC 对比**的可复现数字 ——
> 同一台机器、同一条链路、同一个视频,两边各量 fps 曲线、端到端延迟、码率、CPU。
>
> 已经有的是**我们自己两条路之间**的数([12 §9](12-xpra-client.md#9-量到的数)):
> 滚动 3.71 Mbps(其中 57% 的重绘面积由零字节的 `scroll` 包完成)、
> 全屏动画 2.86–9.34 Mbps ≈ 20 fps。**那组数反过来印证了这一节的判断**:
> 全屏持续运动确实是分区域重传的负收益区 —— xpra 在那儿也退化到 9 Mbps。
...
### 5.1 那 0.7.0 加的 `--transport` 算不算开关?

**不算,而且这一节的论证正好解释了为什么。**

上面那张表里,VNC 和 screencast **每一行都不同** —— 输入、权限、帧里有什么、
runtime 契约、`session()` 的参数。留开关等于两套东西同时在线。

xpra 和 screencast 那张表**只有一行不同**:

| | screencast | xpra |
| --- | --- | --- |
| 像素从哪来 | CDP `Page.startScreencast` | xpra 的 damage 区域 |
| 输入 / 权限 / tab / 原生 UI / 日志 / token / 只读 | ← **完全同一套** → | |

所以它不是"两套路径",是**同一套东西换了一个像素来源**
([11 §5](11-xpra.md#5-xpra-只负责像素别的一律不归它))。而且**默认只有一个**:
screencast 是 xpra 装不上时的退路,以及 `remote` 上唯一可能的那个
([11 §6](11-xpra.md#6-默认走哪条))。

> 换个说法:**这一节拒绝的是"画面和输入一起换";xpra 只换了画面。**
> 能只换画面,恰恰是因为 §1 那个判据翻转已经把输入收回到我们手里了。
