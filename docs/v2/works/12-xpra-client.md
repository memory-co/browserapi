...
[11 §7](c-pixels.md) 那个"借用还是自写"的问题,答案是**自写,大约 500 行**。
...
输入不要([11 §2.1](c-pixels.md) 定了不走 xpra),音频/剪贴板/文件传输不要,
UI 不要(我们自己画)。

所以"借用 xpra-html5 的解码部分"这个想法本身是空的 —— **解码部分不存在**,
存在的是我们全都不要的那部分。

## 5. 结论:自写,约 500 行 —— **实际三百出头**
...
写完之后回来对账(**快照,会随修补漂几行 —— 看的是量级不是个位数**):

| | 行数(含注释) | 去掉注释和空行 |
| --- | --- | --- |
| `view/static/rencode.js` | 126 | **93** |
| `view/static/xpra.js` | 295 | **225** |
| | | **≈ 320** |

比估的 500 还少 —— 估多的是 rencodeplus(以为要 250,实际 93,因为**只解不编
全套**:上行那 6 种包用最朴素的显式编码就够,不需要挑最短表示)。

**要紧的是量级**:三百行 vs `xpra-html5` 那五千行,而且那五千行里没有一行是
我们要的解码(§3、§4)。

服务端那两个不在这张表里,因为它们不是"客户端":
[`webmuxd/xpra.py`](../../../webmuxd/xpra.py)(起 xpra)、
[`view/relay.py`](../../../webmuxd/view/relay.py)(代理 + 白名单)。
...
[11 §8](c-pixels.md) 把这个列为第一个未决问题。**测了,是 desktop。**
...
而且这直接推翻了 [11 §4](c-pixels.md) 那条担心:"popup 在 xpra 模式下又变回真窗口了,
`shim.install` 更要紧"。**在 desktop 模式下不成立** —— popup 被合成进同一个窗口,
客户端根本看不见它是个窗口。

## 7. 上行不是"什么都不发",是**六个包**

[11 §2.1](c-pixels.md) 写的是"我们自己的客户端根本不往 xpra 那条连接发任何东西"。
...
> 代理只放行这 6 个包类型,其余一律丢弃并记一条日志。

白名单不是黑名单 —— **新增的 packet 类型默认是被拒的**,漏不了。而且这条
校验在服务端,不再只靠"客户端没写发送代码"。只读([04 §3](b-input.md))
因此是**两层**保证的。
...
**`scroll` 用零字节干掉了 57% 的重绘面积。** 这是 [11 §1](c-pixels.md) 那张表里
...
这就是 [11 §1](c-pixels.md) 说的自适应,方向和我们的 RTT 自适应
...
[11 §3](c-pixels.md) 整节在讲"怎么把 Chrome 的 bar 裁掉"。**测完之后这一节可以删掉大半。**
...
- 不用把鼠标 `y` 加回 `crop_top` —— [11 §3](c-pixels.md) 那个"最容易错而且很隐蔽"的坑**不存在了**
...
- **`auto-refresh`**([11 §1](c-pixels.md) 说的"静止后补一张无损的")在我们
...
- [11](c-pixels.md) —— 本篇是它 §7、§8 的答案,并修正了它的 §2.1、§3、§4、§5
