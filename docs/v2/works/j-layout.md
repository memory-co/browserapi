...
> **这一节有一份后续设计稿:[`docs/v2/modules/`](../modules/)。**
> 那份的结论是 —— **`models` 不是一件事**,所以它不该是一个文件。
> 按"跨哪种边界"拆它只是把同一个错误排整齐;该换的是主轴:
> **按域分(tab / channel / page / session …),层降级成域之间的一条规矩。**
> 依据是量出来的:扁平之后**一次改动平均动 6.2 个文件**,
> 而每个大文件内部**都已经按同一组域切过一遍了**。
