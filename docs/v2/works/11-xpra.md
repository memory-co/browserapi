...
用它最省事。**但那会把 [03 §1](b-input.md) 那个安全收口拆掉。**
...
只读那条([04 §3](b-input.md))因此原样成立:输入全在我们的通道上,
服务端一行判断就能丢掉。

> **代价要说清楚**:xpra 的输入路径(它自己做过的加速、组合键处理、IME)
> 我们一样都不继承,得继续用 [03](b-input.md) 那套自己的翻译。
> 换来的是安全模型不动。

### 2.2 那条 xpra 连接要不要经过我们

**要,反代一下,不直接暴露 xpra 的端口。** 理由是 [04](b-input.md) 那条
...
| 输入为什么是安全收口 | [03 §1](b-input.md) |
| 只读怎么实现 | [04 §3](b-input.md) |
