...
| **`apt-get install -s` 探依赖** | **抄** | 报包名而不是 `.so` 名,而且不用先有浏览器。比 `ldd` 那条严格更好 |
...
| **默认装中文字体** | **不抄** | 我们不碰系统的包管理器([07 §4.3](07-runtime.md#43-系统依赖和字体照抄-playwright-的姿态)),`--with-deps` 才动手。但**探不到就警告**这条要留着 |
