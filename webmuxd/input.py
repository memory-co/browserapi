...
docs/v2/works/b-input.md:观看者能对远端做的事,全部被限制在 CDP 的 `Input`
域这几个命令里。他**拿不到 DOM,执行不了脚本,发不出任意 CDP 命令** ——
能表达的全部意图就是"在某个坐标点一下""按某个键",和坐在真实浏览器前的人
完全等价,不多不少。

所以这个文件里只会出现 `Input.*`,而且是写死的四个([b](../docs/v2/works/b-input.md))。

**键盘用带 `text` 的 `keyDown`,不是 `Input.insertText`**([b](../docs/v2/works/b-input.md)):
...
async def deliver(session: Any, sid: str | None, msg: dict) -> None:
    """观看者的一次输入,送进去。

    **收口就在这儿**:观看端能表达的意图只有 `Input.*` 那几条
    ([b §1](../docs/v2/works/b-input.md#1-收口在哪))。
    """
    if sid is None:
        return
    # **v2 里"是不是人"不用再靠相关性猜** —— 它就是从我们自己这条 WS 进来的
    # (v1 得靠页面探针 + 时间窗口反推,f §3.2)。
    # 只有按下和敲键算,移动和滚轮不算 —— 否则鼠标一动 API 就被让路窗口挡住。
    if msg.get("type") in ("key", "text") or (
            msg.get("type") == "mouse" and msg.get("event") == "down"):
        session.note_human_activity(str(msg.get("type")))
    await Translator(session.cdp, sid).handle(msg)
