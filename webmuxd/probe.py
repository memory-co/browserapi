"""**页面里的探针** —— 装在每个文档上的那一小段 JS。

它回答的是 CDP 答不了的那几个问题:是不是人在动、焦点在哪个控件上、
弹窗被谁挡住了。走 `Runtime.addBinding` 报回来,**全项目只开一个 binding**
([b §6](../docs/v2/works/b-input.md))。

叫 `probe` 不叫 `shim`:shim 说的是手段(垫一层),
而它的身份是**探针** —— 只看不改。

> 一条硬规矩:**探针不许读表单控件的 `value`。**
> 密码框上那就是明文。控件身份从 label / aria-label / placeholder 取。
"""
