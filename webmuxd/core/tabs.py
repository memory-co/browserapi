...
@dataclass
class Tab:
    """api/tabs.md §1 的 Tab 对象。

    `index` 不在这儿 —— 顺序是 TabTable 自己维护的一个列表,
    CDP 没有"把 tab 挪个位置"的命令(works/06 §4)。
    """
    ...
    def to_json(self, *, index: int, active: bool) -> dict[str, Any]:
        return {
            "id": self.id, "index": index, "active": active,
...
class TabTable:
    ...
    def list_json(self) -> dict[str, Any]:
        return {
            "tabs": [self._by_id[i].to_json(index=n, active=i == self._active)
    ...
    async def _adopt(self, info: dict) -> None:
        ...
        async with self._lock:
            ...
            tab = Tab(id=tab_id, target_id=target_id, url=info.get("url", ""),
    ...
    async def open(self, url: str = "about:blank", *, activate: bool = True) -> Tab:
        if is_blocked(url):
    ...
    async def activate(self, tab_id: str) -> Tab:
        """切过去。
