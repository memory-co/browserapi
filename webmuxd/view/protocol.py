"""帧头和上行消息 —— docs/v2/works/02-frame-protocol.md §1。

**28 字节定长头 + 图片裸字节**,不是 JSON。CDP 给的是 base64,原样塞进 JSON
转发要多花 33% 体积和两次编解码([02 §1](../../docs/v2/works/02-frame-protocol.md))。
...
#: ([03 §1](../../docs/v2/works/03-input.md))。
UPSTREAM = frozenset({
    "ack",          # 收到一帧
